...
class HBMMImpl {
	...
	personalize_profile(user_id, user_profile, stmmRes, rbmmRes) {
		...
		var previouse_user_profilePrefs = user_entry.get_user_profile().user_preferences.default.preferences
		...
		if(flag)
			new_user_weigths = this.update_user_weights(user_id, user_profile, user_entry.get_stmm_profile(), user_entry.get_rbmm_profile(), user_weigths)
		...
		for(var [key, value] of this.preference_dimensions) {
		   ...
		   //priority is for recommanded preferences
			...
			else if(key in stmmPrefs && (user_profilePrefs[key] == undefined || user_profilePrefs[key] != stmmPrefs[key])) {
		...
		user_entry.set_user_profile(user_profile)
	...
	update_user_weights(user_id, user_profile, stmm_profile, rbmm_profile, user_weigths) {
		...
		console.log('user['+user_id+']: stmm_similarity ',stmm_similarity, ' rbmm_similarity: ', rbmm_similarity)
	...
	__mpd_file_parser (mpd_content) {
		...
		var json = convert.xml2json(mpd_content, {compact: true, spaces: 4})
